...
<%
 /**
  * @Class Name : DepartmentInfoList.jsp
  * @Description : DepartmentInfo List 화면
...
function fn_egov_select(depNumber) {
	document.getElementById("listForm").depNumber.value = depNumber;
   	document.getElementById("listForm").action = "<c:url value='/departmentInfo/updateDepartmentInfoView.do'/>";
   	document.getElementById("listForm").submit();
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/departmentInfo/addDepartmentInfoView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/departmentInfo/DepartmentInfoList.do'/>";
   	document.getElementById("listForm").submit();
}
...
<div id="content_pop">
	...
	<div id="table">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
													<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.depNumber}"/>')"><c:out value="${result.depNumber}"/></a>&nbsp;</td>
